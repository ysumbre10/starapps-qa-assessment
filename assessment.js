      ...
      tasks: [
        {
          ...
          scenario: 'You have been exploring ShopLab — the demo store in the right panel. Look at the interface, layout, styling, and visual elements across Products, Cart, Order History, and Contact pages.',
          question: 'List every UI or visual bug you can find in ShopLab.\n\nFor each bug give:\n- Bug name\n- Where it appears (page / element)\n- What is wrong\n\nDo not write reproduction steps — just identify and describe each bug clearly.',
          placeholder: 'Bug 1: ...\n  Where: ...\n  What is wrong: ...\n\nBug 2: ...\n  Where: ...\n  What is wrong: ...',
          evalPrompt: 'You are a senior QA engineer scoring a UI bug-finding exercise on a demo e-commerce app (ShopLab). Known UI bugs: (1) Sale badge has low-contrast yellow text on light yellow background — fails WCAG contrast ratio, (2) Remove button in cart uses 10px font — too small to tap on mobile, (3) No order confirmation screen after placing order — only a brief toast notification, cart clears but no summary shown, (4) Error state on contact form uses only a red border — no icon or text label, fails colorblind accessibility. Score 0–10: award 2.5 pts per correctly identified and clearly described UI bug. Award 1–2 pts partial credit for bugs that are partially described or show good awareness. Be generous — candidates may describe bugs differently but still be correct. Return JSON: {"score": N, "feedback": "2–3 sentence summary of what the candidate found and missed"}.'
        },
        {
          title: 'API Bugs',
          scenario: 'Use the API Monitor panel at the bottom of ShopLab to observe network requests and responses as you interact with the store.',
          question: 'List every API-level bug you can identify in ShopLab.\n\nFor each bug give:\n- Bug name\n- What is wrong with the API behaviour (wrong status code, wrong response, missing behaviour, etc.)\n\nDo not write reproduction steps — just identify and describe each API bug.',
          placeholder: 'Bug 1: ...\n  What is wrong: ...\n\nBug 2: ...\n  What is wrong: ...',
          evalPrompt: 'You are a senior QA engineer scoring an API bug-finding exercise on a demo e-commerce app (ShopLab). Known API bugs: (1) GET /api/products/detail returns HTTP 201 — wrong for a read operation, should be 200, (2) POST /api/discount/validate returns success:true and discount:0.15 but the UI never applies the discount — API and UI are out of sync, (3) POST /api/orders has no idempotency protection — double-clicking Place Order creates two separate orders with duplicate data, (4) Session expiry returns 401 Unauthorized but callers silently ignore it — no user feedback or redirect shown. Score 0–10: award 2.5 pts per correctly identified API bug. Award 1–2 pts partial credit for partially described bugs. Candidates may describe bugs slightly differently but still be correct — score the understanding, not exact wording. Return JSON: {"score": N, "feedback": "2–3 sentence summary of what the candidate found and missed"}.'
        },
        {
          title: 'Functional Bugs',
          scenario: 'Interact with ShopLab — browse products, manage the cart, apply discounts, and explore how the app handles edge cases and boundary conditions.',
          question: 'List every functional or logic bug you can find in ShopLab — bugs where the app does the wrong thing or allows invalid actions.\n\nFor each bug give:\n- Bug name\n- What is wrong (what the app does vs. what it should do)\n\nDo not write reproduction steps — just identify and describe each functional bug.',
          placeholder: 'Bug 1: ...\n  What is wrong: ...\n\nBug 2: ...\n  What is wrong: ...',
          evalPrompt: 'You are a senior QA engineer scoring a functional bug-finding exercise on a demo e-commerce app (ShopLab). Known functional bugs: (1) Product quantity counter goes negative — clicking minus below 1 shows 0, -1, -2 with no minimum boundary check, (2) Out-of-stock item (Smart Watch) can be added to cart — the Add to Cart button is not disabled for out-of-stock products, (3) Price sort uses string comparison — "Price: Low to High" incorrectly orders $129.99 before $29.99 because "1" < "2" alphabetically, (4) A hidden $2.00 handling fee is silently added to the cart total with no line item shown — Subtotal and Total differ by $2 with no explanation. Score 0–10: award 2.5 pts per correctly identified functional bug. Award 1–2 pts partial credit for bugs that are partially described. Score the understanding shown, not exact wording. Return JSON: {"score": N, "feedback": "2–3 sentence summary of what the candidate found and missed"}.'
        },
        {
          title: 'Any Other Bugs',
          scenario: 'You have explored ShopLab thoroughly. Now look for any remaining bugs not covered by the UI, API, or core functional categories — form validation issues, edge cases, accessibility problems, or any other unexpected behaviour.',
          question: 'List any remaining bugs in ShopLab that are not UI bugs, API bugs, or core functional bugs.\n\nFor each bug give:\n- Bug name\n- Where it appears\n- What is wrong\n\nDo not write reproduction steps — just identify and describe each bug.',
          placeholder: 'Bug 1: ...\n  Where: ...\n  What is wrong: ...\n\nBug 2: ...\n  Where: ...\n  What is wrong: ...',
          evalPrompt: 'You are a senior QA engineer scoring a catch-all bug-finding exercise on a demo e-commerce app (ShopLab). Known remaining bugs: (1) Contact form submits with blank Name and Email fields — no required field validation, (2) Email field on contact form is type="text" — browser skips email format validation, invalid addresses like "abc" are accepted, (3) Character counter on contact form message field starts at 199 instead of 200 — off-by-one error, (4) Submitting contact form with a duplicate email shows "Message sent!" — the UI only checks HTTP status code 200 and ignores body.success:false. Score 0–10: award 2.5 pts per correctly identified bug. Award 1–2 pts partial credit for bugs showing good QA awareness even if not perfectly described. Score the understanding shown. Return JSON: {"score": N, "feedback": "2–3 sentence summary of what the candidate found and missed"}.'
        }
      ]
    },
